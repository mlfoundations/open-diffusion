...
class ParquetDataset:
    def __init__(
        ...
        **kwargs,
        ...
        # Data loading code
        loading_kwargs = (
            {"num_workers": workers, "pin_memory": True} if use_cuda else {}
        )

        # Data loading code
        self.dataset = self.get_dataset(
            path, tokenizer=tokenizer, train=train, **kwargs
        )
        ...
        if num_processes > 1:
            ...
            self.sampler = DistributedSampler(
                self.dataset, num_replicas=num_processes, rank=rank, shuffle=train
            )
        else:
            shuffle = train
            self.sampler = None
    ...
    def get_dataset(self, url, tokenizer, train, num_examples_to_see, filters, text_dropout=0.0):
        ...
        pipeline.extend(
            [
                ...
                wds.batched(
                    self.batch_size, partial=not train, collation_fn=default_collate
                ),
            ]
        )
        ...
        num_worker_batches = math.ceil(
            num_examples_to_see / (effective_batch_size * self.workers)
        )
    ...
    def get_dataset(self, url, tokenizer, train, num_examples_to_see):
        ...
        pipeline.extend(
            [
                ...
                wds.map(
                    filter_keys(
                        set(["pixel_values", "input_ids", "text_raw", "class_idx"])
                    )
                ),
                wds.map_dict(
                    pixel_values=transform,
...
def dummy_pipe_test():
    ds = WebDataset(
        ...
        tokenizer=CLIPTokenizer.from_pretrained(
            "/fsx/home-vkramanuj/stable-diffusion-v1-5", subfolder="tokenizer"
        ),
